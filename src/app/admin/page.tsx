...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'modules' | 'warriors' | 'withdrawals' | 'branding' | 'sounds' | 'signals'>('analytics');
    ...
    <div className="min-h-screen bg-background text-white pb-32">
      <header className="fixed top-0 inset-x-0 h-20 bg-black/60 backdrop-blur-xl border-b border-white/5 z-[100] px-6 flex items-center justify-between">
         ...
         <div className="flex items-center gap-4">
            ...
            <Badge variant="outline" className="border-white/10 text-white text-[8px] font-black uppercase">v75.0 Profit Node</Badge>
         </div>
      </header>
      ...
      <main className="pt-28 px-4 md:px-6 space-y-10 max-w-7xl mx-auto">
         <div className="flex gap-2 overflow-x-auto no-scrollbar pb-4 sticky top-20 z-50 bg-background/80 backdrop-blur-md pt-2">
            ...
            <NavPill active={activeTab === 'monetization'} label="Economy Control" icon={<DollarSign className="h-3 w-3" />} onClick={() => setActiveTab('monetization')} />
         ...
         {/* 1. REAL-TIME PROFIT & VIDEO ANALYTICS HUB */}
         {activeTab === 'analytics' && (
            <div className="space-y-10 animate-in fade-in duration-500">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <AnalyticsCard 
                    ...
                    desc="Raw revenue from video signals"
                    icon={<DollarSign className="text-green-500" />}
                  />
                  <AnalyticsCard 
                    label="Warrior Dividend" 
                    value={`₹${(stats?.totalUserPayoutsINR || 0).toLocaleString()}`} 
                    desc={`${settings?.userRevenueSharePercent || 10}% distributed share`}
                    icon={<Zap className="text-primary" />}
                  />
                  <AnalyticsCard 
                    label="Admin Net Profit" 
                    value={`₹${(stats?.totalAdminProfitINR || 0).toLocaleString()}`} 
                    desc="Platform maintenance liquidity"
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <Card className="bg-[#0a0a0f] border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-2xl border-2">
                     <h3 className="text-xl font-black uppercase italic flex items-center gap-3"><Video className="text-primary" /> Streaming Analytics</h3>
                     <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-1">
                           <p className="text-[9px] font-black uppercase text-muted-foreground">Total Signal Views</p>
                           <p className="text-3xl font-black italic">{stats?.totalViews?.toLocaleString() || 0}</p>
                        </div>
                        <div className="space-y-1">
                           <p className="text-[9px] font-black uppercase text-muted-foreground">Total Watch Time</p>
                  ...
                  <Card className="bg-[#0a0a0f] border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-2xl border-2 overflow-hidden">
                     <h3 className="text-xl font-black uppercase italic flex items-center gap-3"><Globe className="text-primary" /> Global Viewership</h3>
                     <div className="space-y-4 max-h-40 overflow-y-auto no-scrollbar">
                        {stats?.countryBreakdown ? Object.entries(stats.countryBreakdown).sort((a,b) => b[1] - a[1]).map(([country, count]) => (
                           <div key={country} className="flex justify-between items-center bg-white/5 p-3 rounded-xl">
                              ...
                              <Badge className="bg-primary/20 text-primary border-none text-[10px]">{count} Views</Badge>
                           </div>
                        )) : (
                           <p className="text-xs text-muted-foreground italic">No global signals logged yet.</p>
                        )}
                     </div>
                  </Card>
               </div>
            ...
            <div className="space-y-10 animate-in fade-in duration-500">
               <Card className="bg-primary/5 border-primary/20 p-8 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-8 border-2">
                  <div className="space-y-2 text-center md:text-left">
                     <h3 className="text-2xl font-black uppercase italic tracking-tighter">Global Profit Node</h3>
                     <p className="text-xs text-muted-foreground uppercase font-bold tracking-widest">Calibrate the 100:1 INR scaling factor live.</p>
                  </div>
                  <div className="flex items-center gap-4 w-full md:w-auto">
                     ...
                     <div className="space-y-1 flex-1 md:w-40">
                        <Label className="text-[8px] font-black uppercase text-muted-foreground ml-1">Daily Vid Limit</Label>
                        <Input 
                          type="number" 
         ...
         {/* (Other tabs warriors, withdrawals, branding, sounds, signals remain functional) */}
         {activeTab === 'warriors' && (
            <div className="space-y-10 animate-in fade-in duration-500">
               <Card className="bg-primary/5 border-primary/20 p-10 rounded-[3rem] space-y-8">
                  ...
                  <div className="grid md:grid-cols-3 gap-6">
                     ...
                     <div className="space-y-2">
                        <Label className="text-[9px] font-black uppercase text-muted-foreground ml-1">Amount (Coins)</Label>
               ...
               <Card className="bg-[#0a0a0f] border-white/5 rounded-[3rem] overflow-hidden">
                  <div className="p-8 border-b border-white/5 bg-white/5 flex items-center justify-between">
                     <h3 className="text-sm font-black uppercase tracking-widest italic">Warrior Identity Audit</h3>
                     <Badge variant="outline" className="border-white/10 text-[8px] font-black uppercase">Live Registry</Badge>
                  </div>
