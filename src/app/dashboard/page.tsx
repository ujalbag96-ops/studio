    ...
    <div className="flex min-h-screen bg-background text-white relative overflow-x-hidden">
      <aside className="w-72 border-r border-white/10 bg-white/[0.02] hidden lg:flex flex-col fixed inset-y-0 left-0 z-50 backdrop-blur-3xl">
        ...
        <nav className="flex-1 p-6 space-y-2">
          ...
          <SidebarItem active={false} icon={<Target />} label="Withdrawal Hub" onClick={() => router.push('/shop')} />
          ...
          <SidebarItem active={false} icon={<ShoppingBag />} label="Scholar Hub" onClick={() => router.push('/marketplace')} />
        </nav>

        <div className="p-6 border-t border-white/10">
          <button onClick={handleLogout} className="w-full flex items-center gap-4 px-6 py-4 rounded-xl text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-all font-black uppercase text-[9px] tracking-widest italic">
      ...
      <main className="flex-1 lg:ml-72 p-8 md:p-12 lg:p-16 space-y-12 pb-32">
        <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-10">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
               <Badge className="bg-primary/10 text-primary border-primary/20 font-black text-[9px] px-4 py-1.5 uppercase tracking-widest flex items-center gap-2 shadow-xl backdrop-blur-md italic">
                  <ShieldCheck className="h-3 w-3" /> Identity Verified: {profile?.country || 'Global Node'}
               </Badge>
            </div>
          ...
          <div className="bg-white/[0.03] border border-white/10 px-8 py-5 rounded-3xl backdrop-blur-2xl shadow-xl">
             ...
             <div className="flex items-center gap-4">
                <Crown className="h-5 w-5 text-amber-500 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)] animate-pulse" />
                <span className="text-lg font-black text-white italic uppercase tracking-tight">{profile?.rank || 'Bronze I'} Warrior</span>
             </div>
          </div>
        </header>
        ...
        <section className="space-y-6">
           <div className="flex items-center justify-between px-2">
              <h3 className="text-xl font-black uppercase flex items-center gap-4 italic tracking-tight"><LineChart className="h-5 w-5 text-primary" /> Revenue Integrity</h3>
           ...
           <div className="glass-panel rounded-[3rem] p-12 grid lg:grid-cols-12 gap-12 items-center border-2">
              <div className="lg:col-span-7 space-y-8">
                 <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-[0.4em] italic">Daily Platform Gross Yield</p>
                    <h4 className="text-7xl font-black italic text-white tabular-nums tracking-tighter drop-shadow-lg">${platformRevenueUSD.toFixed(2)}</h4>
                 </div>
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 pt-4">
                    <div className="space-y-2 border-l-2 border-white/10 pl-6">
                       ...
                       <p className="text-xl font-bold text-muted-foreground italic tracking-tight opacity-40">${(platformRevenueUSD * 0.7).toFixed(2)}</p>
                    </div>
                    <div className="space-y-2 border-l-2 border-primary/20 pl-6">
                       <p className="text-[8px] font-black uppercase text-primary tracking-widest italic">Student Share (30%)</p>
                       <p className="text-xl font-bold text-primary italic tracking-tight shadow-primary/20">${(platformRevenueUSD * 0.3).toFixed(2)}</p>
                    </div>
                 </div>
              </div>

              <div className="lg:col-span-5 bg-white/[0.05] border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-3xl flex flex-col justify-center text-center space-y-8 relative overflow-hidden group shadow-2xl">
                 ...
                 <div className="space-y-2 relative z-10">
                    ...
                    <h5 className="text-6xl font-black italic text-green-500 tabular-nums tracking-tighter shadow-green-500/20">${userShareUSD.toFixed(2)}</h5>
                    <p className="text-[8px] font-bold text-muted-foreground uppercase mt-3 tracking-widest italic">S2S Postback Verified Signal</p>
                 </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-12 pt-8">
          <div className="xl:col-span-2 space-y-12">
            <section className="space-y-6">
               <h3 className="text-xl font-black uppercase flex items-center gap-4 italic tracking-tight"><Zap className="h-5 w-5 text-amber-500 animate-pulse" /> Operational Daily Feed</h3>
...
function CompactWalletRow({ label, value, country, icon, color }: any) {
  const colors = {
    blue: "border-blue-500/20 text-blue-400 bg-blue-500/5 shadow-blue-500/5",
    amber: "border-amber-500/20 text-amber-500 bg-amber-500/5 shadow-amber-500/5",
    green: "border-green-500/20 text-green-500 bg-green-500/5 shadow-green-500/5",
    primary: "border-primary/20 text-primary bg-primary/5 shadow-primary/5"
  };
  return (
    <div className={cn("p-8 rounded-[2rem] border-2 transition-all hover:scale-[1.02] group", colors[color as keyof typeof colors])}>
       <div className="space-y-5">
          <div className={cn("h-12 w-12 rounded-xl flex items-center justify-center border group-hover:rotate-12 transition-transform shadow-xl", colors[color as keyof typeof colors])}>
             {icon}
          </div>
          <div>
             ...
             <p className="text-[7px] font-bold opacity-30 uppercase tracking-[0.3em] mt-1">{value.toLocaleString()} Signals Synced</p>
          </div>
       </div>
    </div>
  );
   ...
   return (
      <li className={cn("flex items-center justify-between text-[9px] font-black uppercase tracking-widest italic", active ? "text-white" : "text-muted-foreground opacity-40")}>
         ...
         <div className="status-pulse">
            ...
            <span className={cn("relative inline-flex rounded-full h-2 w-2", active ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "bg-red-500")} />
         </div>
      </li>
   );
}
...
function SidebarItem({ active, icon, label, onClick }: any) {
  return (
    <button onClick={onClick} className={cn("w-full flex items-center gap-6 px-8 py-4 rounded-xl transition-all duration-300 font-black uppercase text-[9px] tracking-widest italic", active ? "bg-primary text-white shadow-xl" : "text-muted-foreground hover:bg-white/[0.05] hover:text-white")}>
