    ...
    <div className="max-w-6xl mx-auto p-4 md:p-10 space-y-16 pb-32">
      <header className="space-y-6 pt-12 text-center">
        <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-primary/10 border border-primary/20 shadow-xl">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary italic">Industrial Node Overview v5.0</span>
        </div>
        ...
        <p className="text-muted-foreground font-medium text-lg max-w-2xl mx-auto leading-relaxed">
          The CampusCompanion architecture is built on five core high-performance pillars designed for student success and regional profitability.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <FeatureCard 
          icon={<GraduationCap />} 
          title="Study Locker" 
          desc="Regional academic materials (India only) integrated with AI-powered retention quizzes." 
          badge="EXCLUSIVE"
        />
        <FeatureCard 
          icon={<Globe />} 
          title="Geo-Calibration" 
          desc="Automatic currency switching ($/£/₹) and 30-35% revenue share logic for global nodes." 
          badge="DYNAMIC"
        />
        <FeatureCard 
          icon={<Lock />} 
      ...
      <section className="bg-primary/5 border border-primary/20 rounded-[3rem] p-10 md:p-20 relative overflow-hidden">
        ...
        <div className="relative z-10 grid lg:grid-cols-2 gap-12 items-center">
           <div className="space-y-6">
              <h2 className="text-4xl md:text-5xl font-black uppercase italic text-white leading-tight">The 30-35% <br /><span className="text-primary">Revenue Pipeline</span></h2>
              <p className="text-muted-foreground font-medium leading-relaxed">
                Standard users earn a base 30% revenue share. By hitting the 1,000-user "Mega Milestone", your node is upgraded to the **Elite 35% Tier**, maximizing every single conversion in your network.
              </p>
              <div className="flex flex-wrap gap-4">
                 <Badge className="bg-white/10 text-white border-none font-black px-4 py-2 uppercase text-[10px]">India: 30% Base</Badge>
                 <Badge className="bg-white/10 text-white border-none font-black px-4 py-2 uppercase text-[10px]">Global: 32% Base</Badge>
                 <Badge className="bg-amber-500 text-black border-none font-black px-4 py-2 uppercase text-[10px]">Elite: 35% Flat</Badge>
              </div>
           </div>
           <div className="space-y-4 bg-black/40 p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
              <h3 className="text-xl font-black uppercase italic">Withdrawal Protocol</h3>
              <ul className="space-y-4 text-[11px] font-bold text-muted-foreground uppercase tracking-widest leading-relaxed">
                 <li className="flex items-start gap-3"><span className="h-1.5 w-1.5 rounded-full bg-primary mt-1.5 shrink-0" /> Minimum 50,000 Coins (₹500/$50).</li>
                 <li className="flex items-start gap-3"><span className="h-1.5 w-1.5 rounded-full bg-primary mt-1.5 shrink-0" /> VIP 1 verification (15 Tasks) required.</li>
                 <li className="flex items-start gap-3"><span className="h-1.5 w-1.5 rounded-full bg-primary mt-1.5 shrink-0" /> Manual audit of activity logs by admin.</li>
              </ul>
           </div>
        </div>
      </section>
