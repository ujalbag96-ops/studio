  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-10 space-y-10 pb-32">
      <div className="text-center space-y-4 pt-10">
        <div className="mx-auto h-20 w-20 rounded-[2rem] bg-primary/10 flex items-center justify-center border border-primary/20 shadow-2xl">
        ...
        <p className="text-muted-foreground font-medium uppercase tracking-[0.3em] text-[10px]">Top Warriors of the High-Performance Sector</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4 items-end pb-10">
             ...
             {users?.[0] && <PodiumUser user={users[0]} rank={1} color="text-amber-500" large />}
          ...
          <Card className="bg-[#0a0a0f] border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
             <div className="divide-y divide-white/5">
                {users?.slice(3).map((u, i) => (
                  <div key={u.id} className="p-6 flex items-center justify-between group hover:bg-white/5 transition-all">
                     <div className="flex items-center gap-6">
                        ...
                        <div className="h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 font-black uppercase text-primary">
                           {u.email?.[0] || 'U'}
                        </div>
                        <div>
                           <p className="font-black uppercase italic text-sm text-white">{u.email?.split('@')[0] || 'Warrior'}</p>
                           <p className="text-[10px] font-bold text-muted-foreground uppercase">{u.rank || 'Bronze'} Tier</p>
                        </div>
                     </div>
                     <div className="text-right">
                        <p className="text-xl font-black text-white italic">{(u.coins || 0).toLocaleString()} <span className="text-xs opacity-40">🪙</span></p>
                ...
                {(!users || users.length <= 3) && (
                   <div className="p-20 text-center text-muted-foreground uppercase font-black text-xs">Waiting for more warriors...</div>
                )}
             </div>
          </Card>
        </div>
  ...
  return (
    <div className={cn("flex flex-col items-center gap-4 text-center", large ? "order-2" : rank === 2 ? "order-1" : "order-3")}>
       <div className={cn("relative", large ? "h-28 w-28" : "h-20 w-20")}>
          <div className={cn("absolute inset-0 rounded-3xl border-2 rotate-6 opacity-20", rank === 1 ? "border-primary" : "border-white")} />
          <div className="h-full w-full rounded-3xl bg-[#121216] border border-white/10 flex items-center justify-center shadow-2xl relative z-10 overflow-hidden">
             <span className="text-3xl font-black uppercase text-primary/40">{user.email?.[0] || 'U'}</span>
       ...
       <div className="space-y-1">
          <p className={cn("font-black uppercase italic truncate max-w-[100px] text-white", large ? "text-lg" : "text-xs")}>{user.email?.split('@')[0]}</p>
          <Badge className={cn("bg-white/5 border-white/10 font-black text-[8px]", color)}>{(user.coins || 0).toLocaleString()} 🪙</Badge>
       </div>
    </div>
  );
}
