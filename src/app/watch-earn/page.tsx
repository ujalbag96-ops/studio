...
export default function WatchToEarnMovie() {
  ...
  const triggerAdAndCreditReward = async () => {
    if (!user || isProcessing) return;
    setIsProcessing(true);
    
    toast({ title: "VERIFYING AD SIGNAL", description: "Analyzing industrial 10% share node..." });

    try {
      const res = await fetch('/api/ad-reward', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.uid, type: 'video_stream_ad' })
      });
      const result = await res.json();
      if (result.success) {
        new Audio(SUCCESS_SOUND).play().catch(() => {});
        toast({ 
          title: "10% SHARE CREDITED", 
          description: `+${result.credit} Coins synced to wallet (Distributed Node).` 
        });
      }
    } catch (e) {
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-10 pb-32">
      <div className="flex items-center justify-between pt-10">
         ...
         <div className="flex items-center gap-3">
            <Badge className="bg-primary/20 text-primary border-none uppercase font-black px-4 py-1.5 italic">10% DISTRIBUTED MARGIN ACTIVE</Badge>
         </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-10">
         <div className="lg:col-span-2 space-y-8">
            <div className="relative rounded-[3rem] overflow-hidden bg-black aspect-video border-4 border-white/5 shadow-[0_0_50px_rgba(0,0,0,0.5)] group">
               
               {streamType === 'direct' && isPlaying && (
         ...
         <div className="space-y-8">
            ...
            <Card className="bg-[#121212] border-white/5 p-8 rounded-[2.5rem] space-y-6">
               <h3 className="text-sm font-black uppercase italic flex items-center gap-3 text-white"><AlertCircle className="h-5 w-5 text-red-500" /> Compliance</h3>
